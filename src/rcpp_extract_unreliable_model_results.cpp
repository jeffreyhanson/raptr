	...
	for (std::size_t i=0; i<n_species; ++i) {
      ...
      ++currCol;
			...
			for (std::size_t k=0; k<species_ndp[i]; ++k) {
				...
				minWDist=weightdistMTX(i,j)(k,selected_species_pu_ids[i][0]);
				for (std::size_t m=1; m<selected_species_pu_ids[i].size(); ++m) {
					minWDist=std::min(minWDist, weightdistMTX(i,j)(k,selected_species_pu_ids[i][m]));
				}
