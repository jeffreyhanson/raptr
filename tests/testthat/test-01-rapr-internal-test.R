...
test_that("PolySet conversion function", {
	template<-raster(matrix(1:9, ncol=3), xmn=0, xmx=1, ymn=0, ymx=1, crs=CRS('+proj=merc +lon_0=0 +k=1 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs '))
	polys<-rasterToPolygons(template, n=4)
	pdf1<-raptr:::rcpp_Polygons2PolySet(polys@polygons)
	pdf2<-SpatialPolygons2PolySet(polys)
...
test_that('urap.squared.distance', {
	...
	d2 <- apply(test2.MTX, 1, function(x) {
		min(
			as.matrix(
				dist(
					rbind(
