test_that('rasp (unreliable - default RaspOpts and GurobiOpts)', {
...
test_that('rasp (unreliable - custom RaspOpts and GurobiOpts)', {
  if (!is.GurobiInstalled(FALSE)) skip('Gurobi not installed on system')
  # load data
	data(cs_pus, cs_spp)
	cs_pus<-cs_pus[1:10,]
...
test_that('rasp (reliable - default RaspOpts and GurobiOpts)', {
  if (!is.GurobiInstalled(FALSE)) skip('Gurobi not installed on system')
	data(cs_pus, cs_spp)
	cs_pus<-cs_pus[1:10,]
	# run function
...
test_that('rasp (reliable - custom RaspOpts and GurobiOpts)', {
  if (!is.GurobiInstalled(FALSE)) skip('Gurobi not installed on system')
  # load data
  data(cs_pus, cs_spp)
  cs_pus<-cs_pus[1:10,]
