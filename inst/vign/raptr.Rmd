...
## Data
To map the coarse-scale distributions of the four bird species, we obtained observation data from the Atlas of Living Australia across the whole of Australia (using the _ALA4R R_ package), spatially thinned the data to omit points within 10,000 km$^2$ of each other to ameliorate the effects of sampling bias (using the developmental version of the _spThin_ R package; \url{www.github.com/mlammens/spThin}), and fit 85 % minimum convex polygons (using the _adehabitatHR_ _R_ package). We used this method because it is entirely reproducible using freely available data. The code used to generate this maps is available in the [GitHub repository for the scientific article accompanying this R package](https://github.com/jeffreyhanson/raptr-manuscript).
