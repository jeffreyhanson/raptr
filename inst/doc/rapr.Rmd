...
In the unreliable formulation, the control variables are the $BLM$, $\hat{t}_{s}$, and $\bar{t}_{sa}$ variables.
...
The decision variables are the $X_j$ and $Y_{fsi}$ variables.
...
In the reliable formulation, the control variables are the $BLM$, $\hat{t}_{s}$, $\bar{t}_{sa}$, $R$, and $M$ variables. The decision variables are the $X_j$, $Y_{fsijr}$, $P_{fsijr}$ variables.
...
& & \text{s.t. } & \text{(2b)}\\
%
& & & 1 - \frac{\sum_{i=0}^{I-1} \sum_{j=0}^{J-1} \left( \lambda_{fsi} d_{fsij} P_{fsijr} Y_{fsij} \right)^2}{\sum_{i=0}^{I-1} \left( \lambda_{fsi} \bar{d}_{fsi} \right)^2 } \geq \hat{T}_{fs}  & \forall & 0 \leq f \leq F-1, \tag{5a}\\
& & & & & 0 \leq s \leq S-1\\
%
